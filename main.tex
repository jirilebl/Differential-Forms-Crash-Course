...
%\section*{In ${\mathbb{R}}^n$}
%
%In $n$ coordinates, the whole setup is the same except that
%there are more differential forms.  If $x_1$, $x_2$, \ldots, $x_n$
%are the coordinates, then we have
%\[
%dx_1, \quad dx_2, \quad \ldots, \quad dx_n
%\]
%as our basic one-forms.
%Then the two-forms are expressions such as
%\[
%f_1  \, dx_1 \wedge dx_2
%+
%f_2\,
...
%\cdots
%\]
%Note now that there are actually a lot more coefficients
%in two forms (as long as $n$ is bigger than 3).
%In fact, there are $\frac{n(n+1)}{2}$ coefficients
%as that's how many basic one forms $dx_{j} \wedge dx_{k}$ there are.
%The derivative is still computed the same way.  That is
%\[
%d(f \, dx_{k_1} \wedge dx_{k_2} \wedge \cdots \wedge dx_{k_\ell})
%=
%df \wedge dx_{k_1} \wedge dx_{k_2} \wedge \cdots \wedge dx_{k_\ell} .
%\]
%
%FIXME:
...
dz & = d(z) = dz .
\end{align*}
Let us suppose we wish to compute
\[
\int_C (x^2+y^2) \, dx + z \, dy + dz
\]
where $C$ is the spiral given by $x = \cos t$, $y = \sin t$, $z = t$ for $0 \leq t \leq 2 \pi$.
We can easily compute this using cartesian to be $2 \pi$, but let us for the sake of the example
